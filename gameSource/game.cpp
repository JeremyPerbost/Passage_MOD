...
void createScreen() {
    ...
    int displayW = fullScreen ? screenWidth : 640;
    int displayH = fullScreen ? screenHeight : 510; // ou une autre valeur comme 720
...
int mainFunction( int inArgCount, char **inArgs ) {
    ...
    maxBlowUpFactor = (screenWidth*1) / width;
...
char playGame() {
    ...
    */
    ...
    while( !done ) {
        ...
        if( getKeyDown( SDLK_0 ) ) {
            playerYouthMode = !playerYouthMode;
            printf("YOUTH MODE: %s\n", playerYouthMode ? "ON" : "OFF");
            if(playerYouthMode) {
                setCharacterAges(0.2); // Âge jeune mais pas trop au début
            }
        }
        if( getKeyDown( SDLK_LEFT ) || getJoyPushed( SDL_HAT_LEFT ) ) {
            char notBlocked = true;
        ...
        if(playerYouthMode) {
            ...
            // Calcul normal de l'âge basé sur la position
            age = ( playerX - dX ) / width;
        }
